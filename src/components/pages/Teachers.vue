    ...
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h5">Deseja apagar docente?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Última atualização', value: 'ultima_atualizacao' },
  ...
  methods: {
    async initialize() {
      this.desserts = await this.$store.dispatch('teachers/getTeachers')
    },
    ...
    async deleteItemConfirm() {
      await this.$store.dispatch('teachers/deleteTeachers', this.editedItem.id)
      this.initialize()
      this.closeDelete()
    },
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        const result = await this.$store.dispatch('teachers/editTeachers', {id: this.editedItem.id, data: data})
        console.log(result)
      } else {
        const result = await this.$store.dispatch('teachers/addTeachers', data)
        console.log(result)
      }
      this.close()
      this.initialize()
